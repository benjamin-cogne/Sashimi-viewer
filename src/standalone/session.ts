...
export interface SessionSample { name: string; file: string; index: string; size: number; kind: 'bam' | 'cram' }
...
export interface SampleLike { id: number; name: string; kind: 'bam' | 'cram'; file: { name: string; size: number }; index: { name: string } }
...
export function buildSession(args: {
  ...
  return {
    app: SESSION_APP, version: SESSION_VERSION, saved: new Date().toISOString(), build,
    samples: samples.map(s => ({ name: s.name, file: s.file.name, index: s.index.name, size: s.file.size, kind: s.kind })),
...
export function parseSession(text: string): SessionFile {
  ...
  return { app: SESSION_APP, version: raw.version, saved: String(raw.saved || ''), build, samples, fasta: raw.fasta && typeof raw.fasta.file === 'string' ? raw.fasta : null, gene, viewer };
}

/** Which session samples are present among the loaded files (by file name, else by index name), and which are still missing. */
export function matchSession(session: SessionFile, samples: SampleLike[]): { matched: { entry: SessionSample; sample: SampleLike }[]; missing: SessionSample[] } {
  ...
  for (const entry of session.samples) {
    const hit = samples.find(s => !used.has(s.id) && (s.file.name === entry.file || (!!entry.index && s.index.name === entry.index)));
