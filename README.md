...
| Would skipping this exon keep the reading frame? | The coding exons of the model are drawn as **puzzle pieces**: each end is shaped by its codon phase (0 flat, 1 round tab or socket, 2 pointed). An exon whose two ends match can be skipped in frame; two ends that lock together are joined in frame, whatever lies between them. Click an exon for its coding length and phases. |
